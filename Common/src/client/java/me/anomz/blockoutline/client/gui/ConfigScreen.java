...
public class ConfigScreen extends Screen {
    ...
    private void initOutlineTab(int x, int y, int w) {
        CycleButton<OutlineStyle> styleButton = CycleButton
                .builder((OutlineStyle s) -> Component.translatable(s.translationKey()), working.outlineStyle)
                .withValues(OutlineStyle.values())
                .create(Component.translatable("blockoutlinecustomizer.option.style"),
                        (btn, value) -> working.outlineStyle = value);
        placeWidget(styleButton, x, y, w);
    ...
    private void initFillTab(int x, int y, int w) {
        this.addRenderableWidget(Checkbox.builder(Component.translatable("blockoutlinecustomizer.option.fill_enable"), this.font)
                .pos(x, y)
                .selected(working.fillEnabled)
                .onValueChange((cb, v) -> working.fillEnabled = v)
                .build());
        y += rowStep;
    ...
    private void initEffectsTab(int x, int y, int w) {
        ...
        this.addRenderableWidget(Checkbox.builder(Component.translatable("blockoutlinecustomizer.option.gradient"), this.font)
                .pos(x, y)
                .selected(working.outlineRgbGradient)
                .onValueChange((cb, v) -> working.outlineRgbGradient = v)
                .build());
        y += rowStep;
        ...
        this.addRenderableWidget(Checkbox.builder(Component.translatable("blockoutlinecustomizer.option.sync_rgb"), this.font)
                ...
                .onValueChange((cb, v) -> working.syncRgb = v)
                .build());
        y += rowStep;
    ...
    private void initGeneralTab(int x, int y, int w) {
        this.addRenderableWidget(Checkbox.builder(Component.translatable("blockoutlinecustomizer.option.enable_custom"), this.font)
                ...
                .onValueChange((cb, v) -> customOutlineEnabled = v)
                .build());
        y += rowStep;
        ...
        CycleButton<String> presetButton = CycleButton
                .builder(ConfigScreen::presetName, selectedPreset)
                .withValues(new ArrayList<>(config.presets.keySet()))
                .create(Component.translatable("blockoutlinecustomizer.preset"),
                        (btn, value) -> selectedPreset = value);
        placeWidget(presetButton, x, y, w);
    ...
    private void placeWidget(net.minecraft.client.gui.components.AbstractWidget widget, int x, int y, int w) {
        widget.setX(x);
        widget.setY(y);
        widget.setSize(w, widgetH);
    }
    ...
    private int addToggleWithSpeed(int x, int y, int w, Component label,
        ...
        this.addRenderableWidget(Checkbox.builder(label, this.font)
                .pos(x, y)
                .selected(selected)
                .onValueChange((cb, v) -> onToggle.accept(v))
                .build());
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.outline(panelX, panelY, PANEL_WIDTH, panelH, PANEL_BORDER);
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (hexBox != null) {
            ...
            graphics.outline(swatchX, swatchY, widgetH, widgetH, WIDGET_BORDER);
        }
        ...
        if (tab == Tab.EFFECTS && working.rgbColors.isEmpty()) {
            ...
            graphics.outline(paletteRowX, paletteRowY, paletteRowW, h, WIDGET_BORDER);
        }
    ...
    private void drawPreviewCube(GuiGraphicsExtractor graphics) {
        int cx = panelX / 2;
        ...
        graphics.outline(cx - card / 2, cy - card / 2, card, card, PANEL_BORDER);
        graphics.centeredText(this.font, this.title, cx, cy - card / 2 - 14, 0xFFFFFFFF);
    ...
    private static void fillQuad(GuiGraphicsExtractor graphics, float ox, float oy, float ux, float uy, float vx, float vy, int color) {
        Matrix3x2fStack pose = graphics.pose();
        pose.pushMatrix();
        pose.mul(new Matrix3x2f(ux, uy, vx, vy, ox, oy));
        graphics.fill(0, 0, 1, 1, color);
        pose.popMatrix();
    }

    /** Draws one cube edge split into styled segments (each edge is one block-length). */
    private void styledEdge(GuiGraphicsExtractor graphics, float x1, float y1, float x2, float y2,
    ...
    private static void lineSegment(GuiGraphicsExtractor graphics, float x1, float y1, float x2, float y2, float px, int color) {
    ...
    private static class SwatchButton extends AbstractButton {
        ...
        protected void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
            ...
            graphics.outline(getX(), getY(), getWidth(), getHeight(), isHovered() ? WIDGET_BORDER_HOVER : WIDGET_BORDER);
        }
    ...
    private static class StyledSlider extends AbstractSliderButton {
        ...
        public void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
            ...
            graphics.outline(x, y, w, h, isHovered() ? WIDGET_BORDER_HOVER : WIDGET_BORDER);
            graphics.centeredText(font, getMessage(), x + w / 2, y + (h - 8) / 2, 0xFFFFFFFF);
        }
    }
